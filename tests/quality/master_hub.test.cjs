...
function testIntegrationPoints() {
  ...
  assert.match(dashboardHtml, /if \(groupKey === 'despesas'\) \{[\s\S]*?const payslipRows = group\.filter\(row => row\.payslipRole\);[\s\S]*?const expenseRows = group\.filter\(row => fin2IsExpenseItemRow\(row\)\);[\s\S]*?const groupedExpenseRows = fin2ExpenseGroupConfigs\(\)\.flatMap\(config => group\.filter\(row => fin2ExpenseGroupKind\(row\) === config\.key\)\);[\s\S]*?return payslipRows\.concat\(groupedExpenseRows, customRows, expenseRows\);/, 'Financas should keep Despesas PM, fixed subtabs and custom subtabs above ordinary expenses');
  assert.match(dashboardHtml, /if \(groupKey !== 'dividas'\) \{[\s\S]*?return customRows\.concat\(normalRows\);/, 'Financas should render custom subtabs before ordinary rows outside Dividas');
  ...
  assert.match(dashboardHtml, /const regularDebtRows = group\.filter\(row => !fin2IsPayslipDebtRow\(row\) && !fin2IsOldDebtRow\(row\) && !fin2RowCustomGroupKey\(groupKey, row\)\);/, 'Financas should keep ordinary debts outside old and custom debt subtabs by default');
  ...
  assert.match(dashboardHtml, /const orderedDebtRows = fin2DebtGroupOrder\(\)\.flatMap\(kind => debtRowsByKind\[kind\] \|\| \[\]\);[\s\S]*?return orderedDebtRows\.concat\(customRows\);/, 'Financas should render debt groups in their persisted order and include custom debt subtabs');
  ...
  assert.match(dashboardHtml, /function fin2ReceitasOffsetValue\(monthIdx\) \{\s*return 0;\s*\}/, 'Financas legacy Receitas offset helper should be neutral after the single-strip simplification');
  ...
  assert.match(dashboardHtml, /return base;/, 'Financas group totals should follow only the included cells of their own group');
  ...
  assert.match(dashboardHtml, /return raw\.includes\('exclude'\) \? 'exclude' : '';/, 'Financas should collapse legacy combined modes into the single exclude mode');
  ...
  assert.match(dashboardHtml, /function fin2CellModeHas\(mode, flag\) \{\s*return flag === 'exclude' && fin2CellModeValue\(mode\) === 'exclude';\s*\}/, 'Financas should read only the single exclude mode');
  ...
  assert.match(dashboardHtml, /function fin2CellExcludedFromGroupTotals\(groupKey, row, monthIdx\) \{[\s\S]*?fin2CellModeHas\(mode, 'exclude'\)/, 'Financas single-strip mode should zero only the source cell total when excluded');
  ...
  assert.match(dashboardHtml, /function fin2RowsColumnReceitasByCellMode\(rows, monthIdx\) \{\s*return false;\s*\}/, 'Financas subgroup Receitas deduction state should be disabled');
  ...
  assert.match(dashboardHtml, /fin2SetSheetCellTotalMode\(row, monthIdx, fin2CellModeHas\(current, 'exclude'\) \? '' : 'exclude'\);/, 'Financas single strip should switch only local inclusion/exclusion');

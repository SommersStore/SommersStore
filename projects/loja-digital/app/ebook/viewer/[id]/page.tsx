  ...
  return (
    <div className="viewer-container" style={{ background: '#050508' }}>
      ...
      <div className="ebook-wrapper" style={{ 
        ...
        {/* CAPÍTULOS E FÓRMULAS DINÂMICAS */}
        {currentChapters.map((chapter: Chapter, cIdx: number) => {
          const formulasBefore = currentChapters.slice(0, cIdx).reduce((acc: number, c: Chapter) => acc + c.formulas.length, 0);
              ...
              {chapter.formulas.map((item: Formula, fIdx: number) => {
                ...
                return (
                  <React.Fragment key={item.id}>
                    ...
                    {/* Lógica de página extra para fórmulas complexas (Ex: F16) */}
                    {item.id === "16" && (

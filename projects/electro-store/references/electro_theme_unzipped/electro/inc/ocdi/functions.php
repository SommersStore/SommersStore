...
function electro_ocdi_import_files() {
	...
	$dd_path_home_v16 = trailingslashit( get_template_directory() ) . 'assets/dummy-data/home-v16-v17/';
	...
	$import_files[] = array(
		...
		'import_notice'                => wp_kses_post( 'Import process may take 5 minutes. Make sure that the MAS Elementor plugin activated. If you facing any issues please contact our support.<br> After importing the dummy data please import <a href="https://docs.madrasthemes.com/electro/topics/set-up-pages/elementor-kit-upload/"><strong>elementor kit </strong></a>
to display pages just like demo ', 'electro' ),
		'preview_url'                  => 'https://electro.madrasthemes.com/3x/',
		'plugin_profile'               => 'mas-elementor',
		'skip_menu_import'			   => true,
	);

	$import_files[] = array(
		'import_file_name'             => 'Home v16 - v17 Elementor',
		'categories'                   => array( 'Elementor' ),
		'local_import_file'            => $dd_path_home_v16 . 'el-template.xml',
		'import_preview_image_url'     => trailingslashit( get_template_directory_uri() ) . 'assets/images/electro-home-v17-preview.jpg',
		'import_notice'                => wp_kses_post( 'Import process may take 5 minutes. Make sure that the MAS Elementor plugin activated. If you facing any issues please contact our support.<br> After importing the dummy data please import <a href="https://docs.madrasthemes.com/electro/topics/set-up-pages/elementor-kit-upload/"><strong>elementor kit </strong></a>
to display pages just like demo ', 'electro' ),
		'preview_url'                  => 'https://electro.madrasthemes.com/4x/',
		'plugin_profile'               => 'mas-elementor',
		'skip_menu_import'			   => true,
	);
...
function electro_ocdi_after_import_setup( $selected_import ) {
	...
	// Import WPForms
	electro_ocdi_import_wpforms( $selected_import );
}
...
function electro_ocdi_import_wpforms( $selected_import ) {
	if ( ! function_exists( 'wpforms' ) ) {
		return;
	}
	$forms = [];

	if ( 'Home v16 - v17 Elementor' === $selected_import['import_file_name'] ) {
		$forms[] = [
			'file' => get_template_directory() . '/assets/dummy-data/home-v16-v17/wpforms/wpforms-newsletter-16.json'
		];
	} else {
		$forms = [
			[
	...
	foreach ( $forms as $new_form ) {
		$form_datas = json_decode( file_get_contents( $new_form['file'] ), true );

...
int MaxShift()
{
   int bars = Bars;
   if(bars < 20)
      return(MathMax(1, bars - 2));
   return(MathMin(LookbackBars, bars - 2));
}
...
{
   return(iTime(Symbol(), Period(), ShiftAt(ratio)));
}

double PriceLevel(double low, double range, double ratio)
{
   return(NormalizeDouble(low + range * ratio, Digits));
}
...
void OnStart()
{
   ...
   int maxShift = MaxShift();
   int hiShift = iHighest(Symbol(), Period(), MODE_HIGH, maxShift, 1);
   int loShift = iLowest(Symbol(), Period(), MODE_LOW, maxShift, 1);
   double high = iHigh(Symbol(), Period(), hiShift);
   double low = iLow(Symbol(), Period(), loShift);
